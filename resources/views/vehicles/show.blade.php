...
@section('title', 'Vehicle — {{ $vehicle->plateno }}')
...
<div class="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
    <div class="flex items-start gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 tracking-tight">{{ $vehicle->plateno }}</h1>
            <p class="text-gray-500 mt-0.5">
                {{ $vehicle->makeofvehicle ?? '' }}
                @if($vehicle->model) · {{ $vehicle->model }} @endif
                @if($vehicle->vehicletype)
                <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-blue-100 text-blue-700">{{ $vehicle->vehicletype }}</span>
                @endif
            </p>
            @if($vehicle->dept_name)
            <p class="text-xs text-gray-400 mt-1"><i class="fas fa-building mr-1"></i>{{ $vehicle->dept_name }}</p>
            @endif
        </div>
    </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <p class="text-3xl font-extrabold text-gray-800 mt-1">{{ number_format($stats['total_inspections']) }}</p>
        @if($firstInspect && $lastInspect && $firstInspect !== $lastInspect)
        <p class="text-xs text-gray-400 mt-1">
            {{ \Carbon\Carbon::parse($firstInspect)->format('M Y') }} –
            {{ \Carbon\Carbon::parse($lastInspect)->format('M Y') }}
        </p>
        @elseif($lastInspect)
        <p class="text-xs text-gray-400 mt-1">{{ \Carbon\Carbon::parse($lastInspect)->format('M d, Y') }}</p>
        @endif
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <p class="text-xs font-semibold text-green-600 uppercase tracking-wider">Passed</p>
        <p class="text-3xl font-extrabold text-green-600 mt-1">{{ number_format($stats['passed']) }}</p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        ...
        <p class="text-3xl font-extrabold text-red-500 mt-1">{{ number_format($stats['failed']) }}</p>
        @if($stats['failed'] > 0 && $stats['total_inspections'] > 0)
        <p class="text-xs text-gray-400 mt-1">{{ round($stats['failed'] / $stats['total_inspections'] * 100, 1) }}% of total</p>
        @endif
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
        <p class="text-xs font-semibold text-purple-600 uppercase tracking-wider">Pass Rate</p>
        <p class="text-3xl font-extrabold {{ $passRate >= 80 ? 'text-green-600' : ($passRate >= 60 ? 'text-yellow-500' : 'text-red-500') }} mt-1">{{ $passRate }}%</p>
        @if($lastInspect)
        <p class="text-xs text-gray-400 mt-1">Last: {{ \Carbon\Carbon::parse($lastInspect)->diffForHumans() }}</p>
        @endif
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
    <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-xs text-gray-400">
            {{ number_format($stats['passed']) }} passed &nbsp;·&nbsp;
            {{ number_format($stats['failed']) }} failed
            @if($stats['pending'] > 0) &nbsp;·&nbsp; {{ number_format($stats['pending']) }} pending @endif
        </span>
    </div>
    @php
    ...
    <div class="w-full bg-gray-100 rounded-full h-3 overflow-hidden flex">
        <div class="bg-green-500 h-3 transition-all duration-500" style="width:{{ $pPct }}%"></div>
        <div class="bg-red-400 h-3 transition-all duration-500" style="width:{{ $fPct }}%"></div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            ...
            @endphp
            @foreach($vFields as [$label, $value, $icon])
            <div class="flex items-start gap-3 px-5 py-3">
                <div class="w-7 flex-shrink-0 text-center mt-0.5">
                    <i class="{{ $icon }} text-gray-300 text-xs"></i>
                </div>
                <div class="min-w-0">
                    <p class="text-xs text-gray-400">{{ $label }}</p>
                    <p class="text-sm font-medium text-gray-800 mt-0.5 break-words">{{ $value ?? '—' }}</p>
                </div>
            </div>
            @endforeach
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            ...
            @endphp
            @foreach($oFields as [$label, $value, $icon])
            <div class="flex items-start gap-3 px-5 py-3">
                <div class="w-7 flex-shrink-0 text-center mt-0.5">
                    <i class="{{ $icon }} text-gray-300 text-xs"></i>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            ...
            @endphp
            @foreach($sFields as [$label, $value, $icon])
            <div class="flex items-start gap-3 px-5 py-3">
                <div class="w-7 flex-shrink-0 text-center mt-0.5">
                    <i class="{{ $icon }} text-gray-300 text-xs"></i>
                </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        ...
        @if($stats['total_inspections'] > 5)
        <a href="{{ route('vehicles.history', $vehicle->id) }}"
           class="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
            View all {{ number_format($stats['total_inspections']) }} <i class="fas fa-arrow-right text-xs"></i>
        </a>
        @endif
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach($inspections->take(5) as $ins)
                @php
                ...
                <tr class="hover:bg-gray-50 transition-colors group">
                    <td class="px-5 py-3.5 whitespace-nowrap">
                        <div class="font-semibold text-gray-800">
                            {{ $ins->inspectdate ? \Carbon\Carbon::parse($ins->inspectdate)->format('Y-m-d') : '—' }}
                        </div>
                        @if($ins->inspectdate)
                        <div class="text-xs text-gray-400 mt-0.5">{{ \Carbon\Carbon::parse($ins->inspectdate)->diffForHumans() }}</div>
                        @endif
                    </td>
                    <td class="px-5 py-3.5">
                        <span class="font-mono text-xs text-gray-700 bg-gray-100 px-2 py-0.5 rounded">{{ $ins->seriesno }}</span>
                    </td>
                    <td class="px-5 py-3.5 text-xs text-gray-500">
                        {{ $typeLabels[$ins->inspecttype ?? ''] ?? ($ins->inspecttype ?? '—') }}
                    </td>
                    <td class="px-5 py-3.5 text-gray-700">{{ $ins->inspector ?? '—' }}</td>
                    <td class="px-5 py-3.5 text-center">
                        @php $t = (int)($ins->inspecttimes ?? 1); @endphp
                        @if($t > 1)
                        <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-orange-100 text-orange-700 text-xs font-bold">{{ $t }}</span>
                        @else
    ...
    <div class="px-6 py-3 bg-gray-50 border-t border-gray-100 text-center">
        <a href="{{ route('vehicles.history', $vehicle->id) }}"
           class="text-sm text-blue-600 hover:text-blue-700 font-medium">
            View all {{ number_format($stats['total_inspections']) }} inspections
